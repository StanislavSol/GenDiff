from gendiff.diff import get_decoder_data
from gendiff.stylish import get_formated_dict
...
def test_generate_diff():
    ...
    for files in files_for_test:
        result_dict = get_decoder_data(*files[:LAST_ELEMENT])
        verification_file = files[LAST_ELEMENT]
        assert get_formated_dict(result_dict) == verification_file
